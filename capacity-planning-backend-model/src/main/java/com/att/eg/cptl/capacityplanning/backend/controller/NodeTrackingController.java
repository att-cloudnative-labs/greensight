...
@RestController
public class NodeTrackingController {
  ...
  public ResponseEntity<RestResponse> getForecastSheetDescriptors(
      HttpServletRequest request,
      @RequestParam(value = "nodeType", required = false) NodeType nodeType,
      @RequestParam(value = "since", required = false) Long updatedAfterTime,
      @RequestParam(value = "id", required = false) List<String> nodeIds) {
    ...
    return RestResponseUtil.createResponse(
        HttpStatus.OK, treeNodeService.getTrackingInfo(user, updatedAfter, nodeIds, nodeType));
  }
}
