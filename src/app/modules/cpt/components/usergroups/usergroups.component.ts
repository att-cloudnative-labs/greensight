            ...
            if (this.userGroupRoleName !== 'ROLE_Read-only') {
                if (this.userGroupRoleName === 'ROLE_Admin') {
                    if (this.userRoleName !== this.userGroupRoleName) {
                        ...
                        this.userService
                            .updateUser(usergroup.usersWithAccess[position].id, usergroup.usersWithAccess[position].username, usergroup.usersWithAccess[position].role, usergroup.usersWithAccess[position].settings)
                            .subscribe(result => {
                                console.log('result', result);
                                if (result.status === 'UNPROCESSABLE_ENTITY') {
                                    this.modal.alert()
