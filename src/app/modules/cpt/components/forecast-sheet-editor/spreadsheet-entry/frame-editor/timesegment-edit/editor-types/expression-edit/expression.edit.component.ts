import { Component, Input, EventEmitter, Output, ElementRef, ViewChild, HostListener, AfterViewInit, OnInit } from '@angular/core';
...
import { AutocompleteInputComponent } from '@cpt/components/auto-complete-input/autocomplete.input.component';
...
export class ExpressionEditComponent implements OnInit, AfterViewInit {
    ...
    @ViewChild('expression', { static: false }) autocompleteComponent: AutocompleteInputComponent;
    @ViewChild('distribution', { static: false }) autocompleteDistComponent: AutocompleteInputComponent;

    @ViewChild('inputExpression', { static: false }) inputExpressionElement: ElementRef;
    @ViewChild(TimeSegDistributionComponent, { static: false }) timeSegDistComponent: TimeSegDistributionComponent;
    @ViewChild('expressionEditor', { static: false }) expressionEditorEl: ElementRef;
    ...
    clickOutside(event: MouseEvent, targetElement: HTMLElement): void {
        ...
        const clickedInside = this._el.nativeElement.contains(targetElement)
            || this.expressionEditorEl.nativeElement.contains(targetElement)
            || targetElement.closest('#editor-target-' + this.sheetId)
