...
import { VariableUnitService } from '../../services/variable-unit.service';
import { UserGroupService } from '../../services/usergroup.service';
import { UserGroup } from '../../interfaces/user-group';
import { Role } from '../../interfaces/role';
import { User } from '../../../login/interfaces/user';
import { VersionService } from '../../services/micro-service-versions.service';
...
export class SettingsComponent implements OnInit {
    ...
    usergroupList: UserGroup[] = Array<UserGroup>();
    ...
    onChangeUserRole(user) {
        if (this.userRole === 'ROLE_Admin') {
            if (user.id !== Utils.getUserId()) {
                this.userService
                    .updateUser(user.id, user.username, user.role, user.settings)
                    .subscribe(result => {
    ...
    */
                ...
                if (this.userGroupRoleName === 'ROLE_Admin') {
                    if (this.userRoleName !== this.userGroupRoleName) {
                        ...
                        if (usergroup.usersWithAccess[position].id !== Utils.getUserId()) {
                            this.userService
                                .updateUser(usergroup.usersWithAccess[position].id, usergroup.usersWithAccess[position].username, usergroup.roleId, usergroup.usersWithAccess[position].settings)
                                .subscribe(result => {
                                    if (result.status === 'UNPROCESSABLE_ENTITY') {
                                        this.modalDialog.alert()
                                            .title('Warning')
    ...
    modifyUserRole(user) {
        ...
        console.log('this.usergroupLis: ' + this.usergroupList.length);
        this.roles.forEach(role => {
        ...
        this.usergroupService.getUserInUsergroups(user.id)
            .subscribe(result => {
                this.usergroupList = result.data as Array<UserGroup>;
                console.log(' usergroupLis length should be empty for each user:' + this.usergroupList.length);
                this.userGroupRolesList = [];
                for (let index = 0; index < this.usergroupList.length; index++) {
                    this.roles.forEach(role => {
                        if (this.usergroupList[index].roleId === role.id) {
                            this.userGroupRolesList.push(role);
                            console.log(user.username + ' current usergroup name: ' + this.usergroupList[index].userGroupName + ' usergrouplist role : ' + this.usergroupList[index].roleId);
                            console.log(user.username + ' is part of ' + this.usergroupList[index].userGroupName + ' user group');
                        }
                ...
                if (user.id !== Utils.getUserId()) {
                    this.userService
                        .updateUser(user.id, user.username, user.roleId, user.settings)
                        .subscribe(result => {
    ...
    removeUserFromUsergroups(userID) {
        this.usergroupService
            ...
            .subscribe(result => {
                ...
                this.userGroupRolesList = [];
                for (let index = 0; index < this.usergroupList.length; index++) {
                    this.usergroupList[index].usersWithAccess.forEach(usergroupUser => {
                        if (userID === usergroupUser.id) {
                            const position = this.usergroupList[index].usersWithAccess.findIndex(accessUser => accessUser.id === userID);
                            this.usergroupList[index].usersWithAccess.splice(position, 1);
                        }
                        this.usergroupService
                            .updateUsergroup(this.usergroupList[index].id, this.usergroupList[index].userGroupName, this.usergroupList[index].usersWithAccess, this.usergroupList[index].roleId)
                            .subscribe(result => {
                                console.log('result of update', result);
                            });
