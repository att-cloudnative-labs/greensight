...
import { PercentileResult } from '../core_module/interfaces/PercentileResult';
...
export class Utils {
    ...
    public static setIsForecastSidebarExpanded(bool) {
        sessionStorage['expanded_forecast_sidebar'] = bool;
    }

    public static getIsForecastSidebarExpanded() {
        if (sessionStorage['expanded_forecast_sidebar'] === undefined) { this.setIsForecastSidebarExpanded(false); }
        return sessionStorage['expanded_forecast_sidebar'];
    }
    ...
    public static setHiddenForecastVariable(varId) {
        let variables = [];
        if (sessionStorage['hidden_forecast_variables'] !== undefined) {
            variables = JSON.parse(sessionStorage['hidden_forecast_variables']);
        }

        const index = variables.findIndex(x => x === varId);
        if (index === -1) {
            variables.push(varId);
            sessionStorage['hidden_forecast_variables'] = JSON.stringify(variables);
        }
    }

    public static getHiddenForecastVariables() {
        return sessionStorage['hidden_forecast_variables'];
    }

    public static removeHiddenForecastVariable(varId) {
        let variables = [];
        if (sessionStorage['hidden_forecast_variables'] !== undefined) {
            variables = JSON.parse(sessionStorage['hidden_forecast_variables']);
        }
    ...
    public static selectProject(id) {
        ...
        sessionStorage['active_project_id'] = id;
    }
    ...
    public static getActiveBranch() {
        return sessionStorage['active_branch_id'];
    }

    public static removeActiveBranch() {
        sessionStorage.removeItem('active_branch_id');
        sessionStorage.removeItem('last_branch_id');
    }

    public static removeActiveProject() {
    ...
    /**
     * Store a specfied simulation in the current session so it can be used as the selected
     * @param simulationId the id of the selected simulation
     */
    public static selectSimulation(simulationId: string) {
        sessionStorage['active_simulation_id'] = simulationId;
    }
