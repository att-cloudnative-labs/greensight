import { Component, Input, Output, EventEmitter, OnInit, ViewChild, ViewChildren, HostListener, ElementRef, QueryList, OnChanges } from '@angular/core';
import { VariableType, TimeSegmentBasic, Variable, Frame, renderProjections, TimeSegment, TimeSegmentBreakdown, Actual, Distribution, DistributionType } from '@cpt/capacity-planning-projection/lib';
import { Utils } from '../../../../../utils_module/utils';
import * as moment from 'moment';
import month = d3.time.month;
...
export class SpreadsheetEntryComponent implements OnInit, OnChanges {
    ...
    /**
     * When the spreadheet is initialized, it pushes some blank rows to the table
     */
    ngOnInit() {
    ...
    handleSelectionMoveKeyboardEvent(event) {
        let nextPosition = Object.assign([], this.seletionPosition);
        let newFrame: any;
        let newFrameVar: any;
        ...
        let newVar: any;
        let newPosition = [];
        if (this.seletionPosition.length === 0) {
            return;
        } else {
            switch (event.which) {
                case 37: {
        ...
        if (nextPosition !== undefined) {
            this.seletionPosition = nextPosition;
        }
        ...
        if (this.currentFrame === false) {
            ...
            const projection = this.uiProjections[this.seletionPosition[0]];
            const frame = projection.frames[this.seletionPosition[1]];
            this.onFrameCellClicked(frame, projection.variable, this.seletionPosition[0], this.seletionPosition[1]);
            ...
            const subFrame = projection.frames[this.seletionPosition[1]].subFrames[this.seletionPosition[0]];
            this.onSubFrameCellClicked(subFrame, projection.variable, this.seletionPosition[0], this.seletionPosition[1], this.frameIndex);
    ...
    */
    ...
    processingVariableCreation(newVariableTitle) {
        ...
        if (this.variableList.length > 0) {
            ...
            color = Utils.getColor(previousVariable.content.color);
        }
        const variable = {
            'content': {
    ...
    */
    ...
    onFrameCellClicked(frame: Frame, variable, rowIndex, colIndex) {
        ...
        this.seletionPosition = [rowIndex, colIndex];
    }
    ...
    onDeletePressed() {
        if (!this.showEdit && this.selection.length > 0) {
