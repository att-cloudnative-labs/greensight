...
import { GraphModel, Process, Inport, Outport, ProcessInterfaceDescription, GraphParam, AspectParam, Aspect, NumberParam } from '@cpt/capacity-planning-simulation-types';
...
export var CptPeReplaceAspectDescription: ProcessInterfaceDescription = {
    ...
    inports: {
        ...
        'ec7e2dbb-79ce-4dbe-b6e6-54e111f11d95': {
            ...
            objectType: 'INPORT',
            requiredTypes: ['BREAKDOWN', 'NUMBER'],
            desiredUnits: [],
    ...
    protected _process() {
        ...
        if (isNumber(a)) {
            ...
            if (isAspect(b)) {
                ...
                let additionalAspect = scaleBreakdown(a.value, b.value);
            ...
            if (isAspectNumber(b)) {
                ...
                for (let aspect of b.aspects) {
                    let additionalAspect = scaleBreakdown(a.value, aspect);
                    out.aspects.push(additionalAspect);
                }
            }
