...
import { Process, Inport, Outport, ProcessPortTemplate, ProcessInterfaceDescription, GraphParam } from '@cpt/capacity-planning-simulation-types';
import { CptEnvironmentIf, CptSimError, CptSimulationProcessIf } from '../cpt-object';
import {
    maxResponseGroups
...
export class CptPeParallelExecution extends CptProcessingElement {
    ...
    protected _processResponse() {
        ...
        let outportB = this.outports[this.OUTPORT_B_ID];
        let extraOutports = this.getTemplatePortInstances(this.OUTPORT_TMPL_ID);
        const responsePorts = [outportA, outportB, ...extraOutports];
        const responses = responsePorts.map(p => { p.processResponse(); return p.yieldResponse() })
        const aggregatedResponse = maxResponseGroups(responses);
