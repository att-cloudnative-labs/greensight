...
import { isNumber, cloneParam, scaleBreakdown, hasAspect } from '../cpt-load-ops';
...
export class CptPeDivideRoundup extends CptProcessingElement {
    ...
    protected _process() {
        ...
        if (isNumber(numerator) && isNumber(divisor)) {
            if (divisor.value === 0) {
                inportDivisor.warn('COMPUTATIONAL_ERROR', 'can\'t divide by zero.');
            } else {
                let result = cloneParam(numerator) as NumberParam;
                ...
                if (hasAspect(numerator)) {
                    result.aspects = [];
                    for (let aspect of numerator.aspects) {
                        result.aspects.push(scaleBreakdown(result.value, aspect));
                    }
                }
