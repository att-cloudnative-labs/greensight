import { CptEnvironmentIf, CptSimulationProcessIf, genSimId, CptSimulationNodeIf } from './cpt-object';
import { Process, NodeTypes, ProcessInterfaceDescription, GraphParam, ProcessInport, ProcessOutport } from '@cpt/capacity-planning-simulation-types';
import { CptInport, CptOutport, } from './cpt-port';
...
export class CptProcessingElement implements CptSimulationProcessIf {
    ...
    protected aggregateResponses(outports: CptOutport[], inports: CptInport[]) {
        ...
        let aggregatedResponse = combineResponseGroups(responses);
        if (aggregatedResponse) {
            for (const inport of inports) {
                for (const rep of aggregatedResponse) {
                    inport.acceptResponse(rep);
                }
    ...
    public init(env: CptEnvironmentIf): boolean {
        ...
        } else if (!this.createDynamicPorts()) {
