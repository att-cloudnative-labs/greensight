...
import { ResponseNumberParam } from "@cpt/capacity-planning-simulation-types/lib";
...
describe("Processing Element: Serial Execution", () => {
    ...
    it("add up responses from A & B with breakdowns", () => {
        pe.process();
        ...
        const response = pe.yieldResponse(pe.INPORT_LOAD_ID);
        const respNum = assertLatencyResponseNumber(response);
        assertSplit(respNum.value, responseEnv.serialAggregate.split);
        // todo check aspects
