...
import { GraphModel, Process, Inport, Outport, ProcessInterfaceDescription, ProcessPortTemplate, GraphParam, AspectNumberParam, Aspect, AspectParam } from '@cpt/capacity-planning-simulation-types';
import { CptSimulationProcessIf } from '../cpt-object';
import { max } from '../cpt-math-ops';
import { isNumber, NumberType, dupl, addAspects, isAspectNumber, applyAspects, isDistNumber, hasUnit, asAspectNumber } from '../cpt-load-ops';
import { hasUnavailableExpressions } from '@cpt/capacity-planning-projection/lib/expression-utils';
...
export class CptPeMaxWeighted extends CptProcessingElement {
    ...
    public process() {
        ...
        try {
            let result = max(numberLoads) as NumberType;
            let aspectLoads: AspectNumberParam[] = loads.filter(load => isAspectNumber(load));
            ...
            if (aspectLoads && aspectLoads.length > 0) {
                ...
                result = asAspectNumber(result);
                result.aspects = applyAspects(result.value, aspects);
            }
