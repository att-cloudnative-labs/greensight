...
export class CptPeAddAspect extends CptProcessingElement {
    ...
    public process() {
        ...
        if (isNumber(a) && isAspect(b)) {
            let out: AspectNumberParam = {
                type: 'ASPECT_NUMBER',
                value: a.value,
                unit: a.unit,
                aspects: a.type === 'ASPECT_NUMBER' && a.aspects ? a.aspects : []
            }
            let additionalAspect = scaleAspect(out.value, b.value);
