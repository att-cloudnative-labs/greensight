...
export class CptGraphModel implements CptSimulationProcessIf {
    ...
    private pushResponseToPrevNodes(destinationId: string, response: GraphParam) {
        let connections: Connection[] = [];
        for (let connectionId in this.connections) {
            let c = this.connections[connectionId];
    ...
    private runInProcessOrder(cb: (CptSimulationActiveNodeIf) => void) {
        for (let node of this.processOrder) {
            cb(node);
        }
    }

    private runInInverseProcessOrder(cb: (CptSimulationActiveNodeIf) => void) {
        for (let node of this.processOrder.reverse()) {
            cb(node);
        }
    }
